...
void main() {
    ...
    // Fix mouse aspect ratio so the effect is a perfect circle
    vec2 mouse = uMouse;
    mouse.x *= uAspect;
    ...
    // Pull the UVs toward the mouse
    vec2 dir = normalize(uv - mouse);
    float effectRadius = 1.5;
    float pullStrength = 1.5;
    float lens = pow(smoothstep(effectRadius, 0.0, dist), 2.0);
    uv *= dir * lens * pullStrength;
